...
import {
  failureAddingCat,
  failureEditingCat,
  successAddingCategory,
  successEditingCategory,
  successAddingProduct,
  failureAddingProduct,
  successEditingProduct,
  failureEditingProduct,
  ...
  try {
    ...
    yield put(
      successAddingProduct({
        ...
        score: 2,
        likes: 0,
        colors: ["#99CCCC", "#FF9933", "#990033"],
        isAvailable: payload.count > 0 ? true : false,
      })
    );
...
export default function* ProductSaga() {
  yield all([
    call(fetchAddCategory),
    call(fetchEditCategory),
    call(fetchAddProduct),
    call(fetchEditProduct),
  ]);
}

const addUrls = (collection) => {
  const res = collection.map((item) => {
    if (item != undefined) return item.data.url;
  });
  return res;
};
