...
export const editData = (collection, newData) => {
  ...
  if (notFound) throw "دسته بندی با این شناسه پیدا نشد!";
  return coll;
};
export const getRelatedProducts = (products, cateoryId) => {
  const relatedProducts = products.filter(
    (item) => item.categoryId == cateoryId
  );
...
export const addNewCategory = (categories, newCat) => {
  ...
  if (isFound) throw "دسته بندی با این شناسه قبلا وجود دارد!";
...
export const addNewProduct = (products, newCat) => {
  ...
  if (isFound) throw "محصول با این شناسه قبلا وجود دارد!";
...
export const editProduct = (collection, newData) => {
  ...
  if (notFound) throw "محصول با این شناسه پیدا نشد!";
  return coll;
};
export const deleteProduct = (collection, id) => {
  let isFound = false;
  const filtered = collection.filter((item) => {
    if (item.productId == id) isFound = true;
