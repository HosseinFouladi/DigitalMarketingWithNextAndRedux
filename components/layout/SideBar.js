...
import {
  ...
  ListImages,
  SideBarBox,
} from "../../styles/layout/SideBar.style";
        ...
        {categories &&
          categories.map((item) => {
            return (
              <CategoryItem
                key={item.categoryId}
                active={id == item.categoryId ? true : false}
                onMouseEnter={() => showProduct(item.categoryId)}
                onClick={(e) => showProductpPage(e, item.categoryId)}
              >
