...
import {
  ...
  PaymentBox,
  PriceBox,
} from "../../styles/product-preview/ProductPayment.style";
...
const ProductPayment = () => {
  ...
  const addToCart = () => {
    ...
    Notif(
      "success",
      "محصول مورد نظر با موفقیت به سبد کالا اضافه شد",
      "اضافه به سبد خرید!"
    );
  };
  return (
    <Payment>
      <PaymentBox>
        <AvailibilityBox>
          <Score>
            {product && product.isAvailable ? "موجود در انبار" : "ناموجود"}
          </Score>
          <DetailIcon icon={faCheck} color="dont-care"></DetailIcon>
        </AvailibilityBox>
        <Explain>
          تمامی مسئولیت های ویژگی خریدازجمله رنگ محصول ِقیمت و....برعهده
          خریدارمیباشدوپس ازپرداخت ِهیچ گونه بازگردانی صورت نمیگیرد
        </Explain>
        <PriceBox>
          <PriceContainer>
          ...
          <ButtonContaier>
            <CartButton
              onClick={addToCart}
              opacity={product.isAvailable ? "bold" : "light"}
              disabled={product && product.isAvailable ? false : true}
            >
              افزودن به سبد خرید
            </CartButton>
          </ButtonContaier>
