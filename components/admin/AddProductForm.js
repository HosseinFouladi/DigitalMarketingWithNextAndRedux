...
import { Form, Subject, Titles,Input, ButtonBox, AddButton, Loader,Circle, ButtonContent, ButtonText } from "../../styles/admin/AddCategory.style"
import { Label, UploadIcon,FileInput, UploadImageBox,ErrorSpan } from "../../styles/register/LoginForm.style";
import { useForm } from "react-hook-form";
import React, { useEffect } from "react";
import { useState } from "react";
import { useCallback } from "react";
import { useDispatch, useSelector } from "react-redux";
import {  deleteproduct, startAddingProduct, startEdditingProduct, } from "../../redux/actions/ProductActions";
import Notif from "../notification/Notification";

import { selectAddErrorMessage, selectaddLoadingProduct, selectAllProducts,  selectEditLoadingProduct } from "../../redux/selectors/ProductSelector";
const AddProductForm=()=>{
    ...
    const handleAddProduct=useCallback((e)=>{
        ...
        const add=async()=>{
            ...
            try{
               dispatch(startAddingProduct({images,productName,categoryId,productId,price,count}));
            }catch(err){
                console.log(err);
            }
        }
    ...
    const handleEditProduct=useCallback((e)=>{
        ...
        const edit=async()=>{
            ...
            try{
               dispatch(startEdditingProduct({images,productName,categoryId,productId,price,count}));
            }catch(err){
                console.log(err);
            }
        }
    ...
    return(
        <Form type='product' onSubmit={handleSubmit(fetchData)}>
           ...
           { type!='delete' && <Input type='text' {...register("productName",{required:true,minLength:2})} />}
            {errors.productName && <ErrorSpan err={'img'}>{errors.productName.type==='requird'?'نام محصول باید وارد شود!':'نام محصول باید بیشتراز دو حرف باشد!'}</ErrorSpan>}
            <Titles>شناسه (ID):</Titles>
            <Input type='number' {...register("productId",{required:true})} />
            {errors.productId && <ErrorSpan err={'img'}>شناسه محصول باید وارد شود!</ErrorSpan>}
            {type!='delete'&&<Titles>دسته بندی مربوطه (ID):</Titles>}
            {type!='delete'&&<Input type='number' {...register("categoryId",{required:true})} />}
            {errors.categoryId && <ErrorSpan err={'img'}>دسته بندی مربوطه باید وارد شود!</ErrorSpan>}
            {type!='delete'&&<Titles>قیمت محصول(به تومان):</Titles>}
            {type!='delete'&&<Input type='number' {...register("price",{required:true})} />}
            {errors.price && <ErrorSpan err={'img'}>قیمت محصول باید وارد شود!</ErrorSpan>}
            {type!='delete'&&<Titles>تعداد موجود:</Titles>}
            {type!='delete'&&<Input type='number' {...register("count",{required:true})} />}
            {errors.count && <ErrorSpan err={'img'}>تعداد محصول باید وارد شود!</ErrorSpan>}
            {type!='delete' && <Titles>عکس های کالا:</Titles>}
            { type!='delete' &&  <UploadImageBox type={'admin'}>
            ...
            <ButtonBox>
               ...
               {editLoadingProduct  && <Loader  viewBox="0 0 66 66" xmlns="http://www.w3.org/2000/svg">
                ...
                <AddButton name='delete' onClick={handleDeleteProduct} onMouseEnter={handlTypeProduct}>حذف</AddButton>
            </ButtonBox>
        </Form>
    )
}
