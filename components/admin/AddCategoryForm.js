...
import {  deleteCategory,  startAddingCat, startEdditingCat } from "../../redux/actions/ProductActions";
import Notif from "../notification/Notification";
import { selectAddErrorMessage, selectaddLoading, selectEditLoading } from "../../redux/selectors/ProductSelector";
...
const AddCategoryForm=()=>{
    ...
    return(
        <Form type='category' onSubmit={handleSubmit(fetchCat)}>
           ...
           { type!='delete' && <Input type='text' {...register("categoryName",{required:true,minLength:2})} />}
            {errors.categoryName && <ErrorSpan err={'img'}>{errors.categoryName.type==='requird'?'نام دسته بندی باید وارد شود!':'نام دسته بندی باید بیشتر از دو حرف باشد!'}</ErrorSpan>}
            <Titles>شناسه (ID):</Titles>
            <Input type='number' {...register("categoryId",{required:true})} />
            {errors.categoryId && <ErrorSpan err={'img'}>شناسه دسته بندی باید وارد شود!</ErrorSpan>}
            ...
            <ButtonBox>
               ...
               {editLoading  && <Loader  viewBox="0 0 66 66" xmlns="http://www.w3.org/2000/svg">
                ...
                <AddButton name='delete' onClick={handleDelete} onMouseEnter={handlType}>حذف</AddButton>
            </ButtonBox>
        </Form>
    )
}
